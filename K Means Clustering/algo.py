...
def GetDataPointsFromFile(filePath):
    ...
    for line in file:
        data_point = line.split() #Split the line into individual data points based on whitespace.
        if len(data_point) == 0: #If the line is empty, skip it.
            continue
        DATASET.append((float(data_point[1]), float(data_point[2]))) #Append the second and third values from the split line as a tuple to the DATASET. These are assumed to be floating-point numbers.
...
points = []
for i in range(K):
    points.append([random.uniform(0, 10), random.uniform(0, 10)]) #Append a randomly generated pair of (x, y) coordinates to the points list. These coordinates are within the range [0, 10).
